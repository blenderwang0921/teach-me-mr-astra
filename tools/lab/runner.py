...
def compiler():
    host_llvm = Path("/opt/homebrew/opt/llvm/bin/clang++")
    default = str(host_llvm) if sys.platform == "darwin" and host_llvm.is_file() else "clang++"
    selected = os.environ.get("CXX", default)
...
""",
        ...
        if compilation["exit_status"]:
            return {
                "supported": False,
                "detail": (path / "compile.log").read_text(errors="replace")[-3000:],
            }
        execution = execute([path / "probe"], path / "run.log", timeout=15)
        return {
            "supported": execution["exit_status"] == 0,
...
def doctor(root):
    ...
    with tempfile.TemporaryDirectory(prefix="lab-doctor-") as temporary:
        for name in ["cmake", "ctest", "ninja", "git"]:
            ...
            tools[name] = {
                "path": executable,
                "version": log.read_text(errors="replace").splitlines()[0],
            }
        ...
        try:
            ...
            tools["compiler"] = {
                "path": compiler(),
                "version": log.read_text(errors="replace").strip(),
            }
...
def ensure_catch(root, commands, log_dir, timeout):
    ...
    pinned = execute(
        ["git", "rev-parse", "--verify", f"{CATCH_COMMIT}^{{commit}}"],
        log_dir / "catch-pin.log",
        timeout,
        cwd=checkout,
    )
    ...
    current = execute(
        ["git", "rev-parse", "HEAD"], log_dir / "catch-head.log", timeout, cwd=checkout
    )
...
def run_cpp(root, source, spec, report_id, preset):
    ...
    try:
        ...
        if not capability["supported"]:
            raise LabError(
                f"Required {preset} C++{spec['cpp_standard']} capability probe failed: {capability['detail']}"
            )
        checkout = ensure_catch(root, commands, log_dir, timeout)
    except LabError as exc:
    ...
    if commands[-1]["exit_status"]:
        return result(
            "build_timeout" if commands[-1]["timed_out"] else "compile_failure",
            2 if commands[-1]["timed_out"] else 1,
        )
    ...
    discovery = execute(
        ["ctest", "--test-dir", build, "--show-only=json-v1"], log_dir / "discovery.json", timeout
    )
    ...
    if discovery["exit_status"] or not names or not expected <= names:
        summary["error"] = (
            f"Missing acceptance tests or failed discovery: {sorted(expected - names)}"
        )
    ...
    for index, seed in enumerate(spec["seeds"]):
        if index:
            commands.append(
                execute(
                    configure + [f"-DLAB_TEST_SEED={seed}"],
                    log_dir / f"seed-{seed}-configure.log",
                    timeout,
                    cwd=FRAMEWORK,
                )
            )
        ...
        command = execute(
            [
                ...
                "--no-tests=error",
                "--timeout",
                str(spec["resource_limits"]["test_timeout_seconds"]),
                "--output-junit",
                junit,
            ],
            log_dir / f"seed-{seed}.log",
            timeout,
        )
        commands.append(command)
        ...
        for case in cases:
            ...
            summary["cases"].append(
                {"name": name, "seed": seed, "passed": not failed, "failure_detail": detail}
            )
        ...
        if any(
            ...
            for marker in ["***Exception", "Subprocess aborted", "***Not Run"]
        ):
