...
from .core import (FRAMEWORK, LabError, contained, digest, files, identifier,
                   now, read_json, tree_hash, valid_id, validate, write_json)
...
def load_exercise(root, exercise_id):
    ...
    if actual != editable | provided:
        raise LabError(f"Declare every exercise file in editable_paths/provided_paths: {sorted(actual ^ (editable | provided))}")
    if any(not (name == "design.md" or name.startswith(("src/", "include/", "experiments/"))) for name in editable):
        raise LabError("Editable paths must be implementation, design, or experiment files; tests and spec are provided")
    checks = spec["acceptance_checks"]
    if len({c["id"] for c in checks}) != len(checks) or len({c["test_name"] for c in checks}) != len(checks):
    ...
    if spec["material_ref"] is not None:
        raise LabError("Upstream material execution is deferred; use a self-contained exercise with material_ref=null")
    return path, spec
...
def contract_hash(root, exercise_id):
    ...
    items = [(f"exercise/{name}", contained(path, name).read_bytes()) for name in spec["provided_paths"]]
    teaching = instructor(root, exercise_id)
    items.extend((f"instructor/{p.relative_to(teaching).as_posix()}", p.read_bytes()) for p in files(teaching)
                 if p.relative_to(teaching).as_posix() != "ready.json")
    for relative in ["CMakeLists.txt", "CMakePresets.json", "cmake/LabExercise.cmake", "pyproject.toml", "uv.lock"]:
        items.append((f"framework/{relative}", (FRAMEWORK / relative).read_bytes()))
    for directory in ["tools/lab", "schemas"]:
        items.extend((f"framework/{p.relative_to(FRAMEWORK).as_posix()}", p.read_bytes())
                     for p in files(FRAMEWORK / directory) if "__pycache__" not in p.parts)
    return digest(items)
...
def require_ready(root, exercise_id, revision=None):
    ...
    if marker["contract_hash"] != contract_hash(root, exercise_id):
        raise LabError("Exercise contract changed; increment revision and run prepare before practicing")
    from .state import report_for
    report = report_for(root, marker["report_id"])
    if (report["target_kind"] != "validation" or report["exit_status"] != 0
            or report["contract_hash"] != marker["contract_hash"]
            or report["exercise_id"] != exercise_id or report["exercise_revision"] != spec["revision"]):
...
def overlay(source, destination, editable):
    ...
    if actual != set(editable):
        raise LabError(f"Variant must supply exactly editable_paths: {source}; difference {sorted(actual ^ set(editable))}")
    for name in editable:
        shutil.copyfile(contained(source, name), contained(destination, name))


def new_report(spec, kind, contract, preset):
    return {"schema_version": 1, "id": identifier(), "timestamp": now(),
            "exercise_id": spec["id"], "exercise_revision": spec["revision"],
            "source_hash": contract, "contract_hash": contract, "target_kind": kind,
            "toolchain": {}, "commands": [], "exit_status": 2, "test_summary": {},
            "seeds": spec["seeds"], "failure_artifacts": [], "duration": 0.0,
            "sanitizer": preset, "usage_if_available": None, "outcome": "incomplete",
            "source_snapshot": None, "student_source_hash": None, "child_reports": []}
...
def run_variant(root, path, spec, contract, kind, preset="debug", variant=None, extra_tests=False):
    ...
    if report["exit_status"]:
        report["test_summary"]["diagnostics"] = [
            ...
            for c in report["commands"] if c["exit_status"]
        ]
...
def prepare(root, exercise_id):
    try:
        ...
        if session["phase"] == "preparing" and session["current_exercise_id"] == exercise_id:
            session.update(phase="blocked", resume_phase="preparing", reason=str(exc),
                           last_report_id=None, version=session["version"] + 1)
...
def _prepare(root, exercise_id):
    ...
    if ready_path.exists():
        ...
        if old["revision"] > spec["revision"] or (old["revision"] == spec["revision"] and old["contract_hash"] != contract):
            raise LabError("Published contract changed without a new revision; increment spec.revision")
    ...
    if set(instructions["semantic_review"]["requirements_reviewed"]) != set(spec["requirements"]):
        raise LabError("Semantic review must cover every requirement, including non-executable promises")
    ...
    if expectation["kind"] == "compile_failure" and (spec["template"] != "debugging" or not expectation["diagnostic"]):
        raise LabError("Compile-failure skeleton requires a debugging exercise and diagnostic substring")
    ...
    for folder in [teaching / "reference", teaching / "skeleton", *[teaching / "mutants" / m for m in mutant_ids]]:
    ...
    if expectation["kind"] == "test_failure":
        ...
        if skeleton["outcome"] != "test_failure" or set(skeleton["test_summary"]["failed"]) != expected:
    ...
    for mutant in instructions["mutants"]:
        report = run_variant(root, path, spec, contract, "mutant", variant=teaching / "mutants" / mutant["id"], extra_tests=True)
        children.append(report)
        expected = {check_names[c] for c in mutant["checks"]}
        if report["outcome"] != "test_failure" or not expected <= set(report["test_summary"]["failed"]):
    ...
    aggregate.update(child_reports=[r["id"] for r in children], duration=time.monotonic() - start,
                     exit_status=2 if failures else 0, outcome="validation_failed" if failures else "ready",
                     test_summary={"failures": failures, "semantic_review": instructions["semantic_review"],
                                   "acceptance_checks": spec["acceptance_checks"]})
    save_report(root, aggregate)
    if not failures:
        write_json(ready_path, {"schema_version": 1, "exercise_id": exercise_id,
                               "revision": spec["revision"], "contract_hash": contract,
                               "report_id": aggregate["id"]}, "ready")
    else:
        ...
        session = load_session(root)
        if session["phase"] == "preparing" and session["current_exercise_id"] == exercise_id:
            session.update(phase="blocked", resume_phase="preparing", reason="; ".join(failures),
                           last_report_id=aggregate["id"], version=session["version"] + 1)
            commit(root, {"learner/session.json": json_text(validate("session", session))})
    return aggregate
...
def check(root, exercise_id, preset=None):
    ...
    reports = [run_variant(root, path, spec, marker["contract_hash"], "student", profile, extra_tests=True) for profile in presets]
    aggregate = new_report(spec, "student", marker["contract_hash"], preset or "required")
    aggregate.update(source_hash=reports[0]["source_hash"], source_snapshot=reports[0]["source_snapshot"],
                     student_source_hash=reports[0]["student_source_hash"],
                     child_reports=[r["id"] for r in reports], duration=sum(r["duration"] for r in reports),
                     commands=[c for r in reports for c in r["commands"]],
                     toolchain=reports[0]["toolchain"], exit_status=max(r["exit_status"] for r in reports),
                     test_summary={"runs": [{"id": r["id"], "preset": r["sanitizer"], "outcome": r["outcome"],
                                             "summary": r["test_summary"]} for r in reports]},
                     failure_artifacts=[p for r in reports for p in r["failure_artifacts"]])
    if any(r["source_hash"] != aggregate["source_hash"] for r in reports) or tree_hash(path) != aggregate["student_source_hash"]:
...
def check_completed(root, item):
    ...
    if ((original["exercise_id"], original["exercise_revision"]) != (item["id"], item["revision"])
            or original["target_kind"] != "student" or original["exit_status"] != 0
            or original["sanitizer"] != "required" or not original["source_snapshot"]):
        raise LabError("Completed record must reference its successful required-profile student snapshot")
    ...
    for preset in ["debug", *spec["sanitizers"]]:
        ...
        report.update(source_hash=original["source_hash"], source_snapshot=snapshot_relative,
    ...
    return {"report_ids": [r["id"] for r in reports], "exit_status": max(r["exit_status"] for r in reports)}
